...
- **Status:** resolved
- **Resolution:** The 2026-07-29 quiet-meadow redesign (`2026-07-29-quiet-meadow-redesign-design.md`) retired the brutalist terminal aesthetic these elements belonged to. `Footer.astro` no longer renders an `.eof` span (the footer is now a meadow/bird illustration), and `404.astro` no longer renders the `<pre class="signage" aria-hidden="true">` block (the 404 page is now "This page went to seed", a garden illustration + copy). Neither element exists in the DOM anymore, so axe has nothing left to flag. The corresponding `axe-baseline.json` skip entries have been drained; verified via `pnpm a11y:audit:primary`.
...
- **Status:** resolved
- **Resolution:** The 2026-07-29 quiet-meadow redesign (`2026-07-29-quiet-meadow-redesign-design.md`) added a real hero section to the home page (`src/pages/index.astro`), with `<h1 id="hero-title">Essays on the web platform, written at a walking pace.</h1>` as the first heading in `<main>`. Axe's `page-has-heading-one` no longer fires and the `single-h1:count=0` entry in `html-checks-baseline.json` has been drained. Verified via `pnpm a11y:audit:primary` and `pnpm a11y:html`.
...
- **Status:** resolved
- **Resolution:** `TagPill.astro`'s `.tag` now carries `padding-block: 5px`, bringing every tag link to a measured 26 × N px box (confirmed via Playwright `getBoundingClientRect`) — clear of the 24 px minimum. Fixing this surfaced a second, more serious bug in `PostCard.astro`: the whole card (title + meta + tags + description + CTA) was wrapped in one `<a>`, with each `TagPill`'s own `<a>` nested inside it. Nested `<a>` is invalid HTML5 — browsers silently split the outer anchor at each nested-anchor boundary (the HTML parser's adoption-agency algorithm), producing multiple small anchor fragments (as little as 20–27 px tall) instead of one full-card target. That fragmentation, not the tag size alone, was the dominant source of axe's `target-size` failures once the home page's post count grew. Fixed by switching `PostCard.astro` to the "stretched link" pattern (`src/components/PostCard.astro`): only the `<h3>` title is a real `<a>`; a `::after` on that anchor, positioned `absolute; inset: 0` against `.post-card` (`position: relative`), stretches its hit area back over the whole card. `TagPill`'s `.tag` gets `position: relative; z-index: 1` so it stays clickable above that overlay. Verified via `pnpm a11y:audit:primary`; the `home:*:target-size:*` entries have been drained from `axe-baseline.json`.
...
- **Status:** resolved
- **Resolution:** Resolved as a side effect of the F-006 fix (Task 12, quiet-meadow redesign): `PostCard.astro` no longer wraps the whole card in one `<a>`. Only `<h3>` wraps a real `<a>`, so its accessible name is exactly the post title — the overlay-technique fix this finding itself suggested, applied for an unrelated but adjacent reason (nested-anchor HTML validity / target size).
- **Maps to:** none
